...
std::vector<FilterConfig> configs = {
  filter_bme688[0], filter_bme688[1], filter_bme688[2], filter_bme688[3], // BME688: Temp, Feuchte, Druck, Gas
  filter_sht45[0], filter_sht45[1],                                       // SHT45: Temp, Feuchte
  filter_gps_neo_m10[0], filter_gps_neo_m10[1], filter_gps_neo_m10[2],   // GPS: Lat, Lon, Höhe
  filter_gps_neo_m10[3], filter_gps_neo_m10[4], filter_gps_neo_m10[5],   // Speed, Kurs, Sats
  filter_mpu6050[0], filter_mpu6050[1], filter_mpu6050[2],               // MPU-6050: Acc X, Y, Z
  filter_mpu6050[3], filter_mpu6050[4], filter_mpu6050[5]                // Gyro X, Y, Z
};
...
void setup() {
  ...
  gpsSerial.write(ubxCfgRate, sizeof(ubxCfgRate)); // NEO-M10: 10 Hz
  Wire.begin();
  if (!bme.begin()) {
...
void loop() {
  // BME688 und SHT45: Alle 30 Minuten
  ...
  SensorData imuData;
  imuData.values = {a.acceleration.x, a.acceleration.y, a.acceleration.z,
                    g.gyro.x, g.gyro.y, g.gyro.z};
  imuData.timestamp = millis();
  imuData.sensorId = "MPU6050";
  filter.pushSensorData(imuData);
  ...
  if (millis() - lastPrint > 200) {
    ...
    Serial.printf("BME688: T=%.1f°C, H=%.1f%%, P=%.1fhPa, G=%.1fkOhm | "
                  ...
                  "MPU: Acc=%.2f,%.2f,%.2fg, Gyro=%.2f,%.2f,%.2f°/s\n",
                  ...
                  filtered[12], filtered[13], filtered[14], filtered[15], filtered[16], filtered[17]);
    lastPrint = millis();
  }
}
